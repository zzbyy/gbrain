...
async function embedBatchWithRetry(texts: string[]): Promise<Float32Array[]> {
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      // Only pass dimensions if using OpenAI models (local models may not support it)
      const createParams: OpenAI.EmbeddingCreateParams = {
        model: MODEL,
        input: texts,
        ...(process.env.GBRAIN_EMBED_MODEL ? {} : { dimensions: DIMENSIONS }),
      };
